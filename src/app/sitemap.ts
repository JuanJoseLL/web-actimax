...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const latestProductUpdate = newestDate(
    products.flatMap((product) => product.updatedAt ?? []),
  );
  ...
  return [
    ...
    ...CATEGORIAS.map((categoria) => ({
      ...
      lastModified: newestDate(
        products
          .filter((product) => product.type === categoria.tipo)
          .flatMap((product) => product.updatedAt ?? []),
      ),
      changeFrequency: "weekly" as const,
      priority: 0.9,
    })),
    ...DEPORTES.map((deporte) => ({
      ...
      lastModified: newestDate(
        products
          .filter((product) => product.deportes.includes(deporte.deporte))
          .flatMap((product) => product.updatedAt ?? []),
      ),
      changeFrequency: "weekly" as const,
      priority: 0.8,
    ...
    { url: `${SITE_URL}/mi-plan/`, changeFrequency: "monthly", priority: 0.7 },
    ...
    ...products.map((product) => ({
      ...
      lastModified: dateOrUndefined(product.updatedAt),
      changeFrequency: "weekly" as const,
      priority: 0.8,
    ...
    ...getBlogCategories().map((category) => ({
      ...
      lastModified: newestDate(
        posts
          .filter((post) => post.tags.includes(category.name))
    ...
    ...posts.map((post) => ({
      ...
      lastModified: dateOrUndefined(post.updatedAt ?? post.date),
